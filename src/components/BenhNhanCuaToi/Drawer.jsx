...
const DrawerBN = (props) => {
    const { openView, setOpenView, dataView, setDataView } = props;
            ...
            <ul>
                {dataView?.patientDetails?.map((item) => {
                    return (
                        <>
                            <li
                                key={item.ngayKhamBenh + item.tenGioKham}
                                style={{ lineHeight: "30px" }}
                            >
                                <strong>Ngày khám:</strong> {item.ngayKhamBenh}{" "}
                                <br />
                                <strong>Giờ khám:</strong> {item.tenGioKham}{" "}
                                <br />
                                <strong>Lí do khám:</strong>{" "}
                                {item.lidokham || ""} <br />
